// WeatherWidget — compact weather chip pinned to the right corner of the
// Marketplace header row, level with "Unified Catalog." and the glass pills.
//
// The animated weather glyph (rotating sun, drifting rain) is unchanged from
// the sidebar capsule it replaces; the shell is resized to sit inline with the
// header buttons — temperature + city at rest, condition and humidity slide
// in on hover.
//
...
// so re-entering the catalog doesn't refetch. If both lookups fail the chip
...
export function WeatherWidget() {
  ...
  return (
    <div
      ...
      className="group relative flex h-9 shrink-0 cursor-default select-none items-center gap-2 overflow-hidden rounded-full border border-white/10 bg-white/5 px-3 font-mono-display text-white backdrop-blur-xl transition-all duration-300 shadow-[inset_0_1px_1px_rgba(255,255,255,0.2),0_10px_30px_rgba(0,0,0,0.35)] hover:border-white/25 hover:bg-white/[0.08]"
    >
      <svg
        viewBox="0 0 64 64"
      ...
      <span className="font-garamond text-2xl font-light leading-none tracking-wide">
        {temp}
      </span>
      <span className="max-w-[110px] truncate text-[9px] uppercase tracking-widest text-white/60">
        {wx?.city || (failed ? 'no signal' : 'locating…')}
      </span>
      {/* condition + humidity reveal on hover; the header row's right anchor
          means the chip widens leftward, never toward the viewport edge */}
      <span className="max-w-0 overflow-hidden whitespace-nowrap text-[9px] uppercase tracking-widest text-white/40 opacity-0 transition-all duration-300 group-hover:max-w-[170px] group-hover:opacity-100">
        {wx ? `· ${describeWmo(wx.code)} · ${wx.humidity}% humidity` : failed ? '· no signal' : ''}
      </span>
    </div>
  )
}
