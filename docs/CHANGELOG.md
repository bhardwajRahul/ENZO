...
### Google Colab — zero-install run path (`notebooks/enzo-colab.ipynb` + `scripts/gen-colab-notebook.py`)

The README's new **Open In Colab** button (after Quickstart) opens a notebook that does the whole setup: clone + `npm ci --include=dev` + frontend build (with `VITE_GOOGLE_AUTH=0` to match the self-hosted build) + background server boot, then prints two URLs — the Colab proxy link (`google.colab.kernel.proxyPort(5001)`, works in the user's own browser) and a Cloudflare quick-tunnel link (trycloudflare.com, works from any device, no account). Every cell is idempotent (a re-run reuses the clone/deps/build and the running server). **Disconnect prevention for 6+ hour runs**: a keep-alive HTML cell resets Colab's ~90-minute idle timer every 60 s (classic connect-button click + title change — works while the tab stays open) and a daemon watchdog thread pings `/api/health` every 5 min and restarts the server if it dies; free Colab's ~12 h cap keeps 6 h comfortably in reach, stated honestly (the tab must stay open). The notebook clones the **public release repo** (theguysudo/ENZO — stable), replicates the docker runtime setup for the one absent file (`src/core/memory-store.json` symlink → `data/`), and all 4 code cells compile-verified; the button goes live once the notebook is pushed. The notebook is generated by `scripts/gen-colab-notebook.py` (edits go in the generator, then re-run it — hand-editing ipynb JSON escapes is how the `\u{...}` bug happened).

### Docker — auto dependencies install at container start (`docker-entrypoint.sh`, new)

When a user pulls and runs the image, the entrypoint now verifies the baked `node_modules` against `package.json` + `package-lock.json` on every start and installs whatever is missing before the server boots — a partial bake, a first boot, or an empty mounted `node_modules` volume all heal automatically from the network. With a complete bake it's a fast no-op (~1s `npm ls` check); `ENZO_AUTO_INSTALL=0` skips the check entirely. `--include=dev` is load-bearing: `NODE_ENV=production` would otherwise make npm skip devDependencies, and `tsx` (the runtime) IS one — the server would never boot without it. `chmod` happens in a Dockerfile layer, not the source mode, so a Windows checkout of a public clone (which loses the exec bit) can't kill the container at boot. `ENTRYPOINT` + `exec "$@"` keeps tsx as PID 1, so `docker stop` signals reach it directly. **Note**: the docker files now live at the repo ROOT (`Dockerfile`, `docker-compose.yml`, `.dockerignore` — moved from `deploy/docker-variant/` in an external restructure; the old `deploy/` release pipeline is gone). Both paths live-tested: complete bake → no install line, health 200 in ~6s; wiped `node_modules` → auto-install fires → `dependencies ready` → health 200.

### In-chat file converter & research-table extractor

Attach a PDF/spreadsheet/csv/json/txt/md in the terminal chat → it's parsed to real text/rows in the browser → the agent extracts, merges, or cross-converts → CSV / Excel download buttons appear on the reply. Files never leave the device; only the reasoning step uses the user's own key, exactly like normal chat. Uncommitted (awaiting review on :5200).

### `src/lib/fileConvert.ts` — new, all client-side

- **Extractors**: PDF text layer via `pdfjs-dist` (worker via Vite's `?worker` suffix) with an x-aligned column heuristic for tables — text items group into lines by baseline y (descending = reading order), cells split where inter-item gaps exceed the line's median glyph run; a table needs ≥2 lines of ≥3 aligned cells with short cells, which keeps a research paper's two-column body text from reading as a table (full text still travels, so a 2-column table arrives as text). Spreadsheets via `exceljs` (browser-field bundle; CJS interop shim covers node + Vite). Native RFC4180 CSV/TSV parser (quotes, embedded delimiters/newlines, BOM, delimiter sniffing). JSON → object rows. Scanned PDFs (no text layer) get an explicit `noTextLayer` marker instead of failing.
- **Cross-format matrix**: `convertAttachment(file, csv|xlsx|json|md|txt)` — extract to rows/text, serialize to target. `mergeTables` unions every detected table under one header (matching headers contribute rows only; different headers are kept as rows, nothing silently dropped).
- **Downloads**: `downloadCsv` / `downloadExcel` (exceljs workbook → Blob → `a[download]`).
- **`selfCheck()`**: round-trips over csv/tsv/markdown/json/xlsx — passing in node (`tsx`), plus an end-to-end matrix run: real CSV → parsed rows → xlsx/json/md, produced xlsx back-reads to identical rows, and a hand-built table PDF extracts to the right 3 columns top-down.
- **Vite**: `optimizeDeps.include` for both packages in `vite.config.ts` so the first attach doesn't trigger a mid-session dependency optimization (which force-reloads the page).

### `TerminalSection.tsx` — 6 edits

1. `parseAttachment` wired into `processFiles` — documents parse the moment they attach (the old path `readAsText`'d every non-image, so PDFs/xlsx attached as garbage bytes; binary kinds now keep `content` empty).
2. `AttachedFile` gains `parsing` / `parsed` state.
3. Attachment chips report the parse state live: `parsing…` → `3 tables · 84 rows` / `no text layer` / `<size> text`.
4. `handleSend` injects parsed content: tables travel as TSV blocks (one per table/sheet, labeled), pdf text is fenced and truncated at 24k chars; unparsed falls back to the old text read.
5. Send-guard: the send holds while any attachment is still parsing, and the send button disables for it.
6. Any assistant reply carrying a markdown table gets CSV / Excel download buttons (all tables in the reply merge into one file) beside the existing Copy/Preview pills.

### Dependencies

`pdfjs-dist` ^4.10.38 + `exceljs` ^4.4.0 (chosen over the registry `xlsx` package, which ships open advisories CVE-2023-30533 / CVE-2024-22363). The only package.json additions of this batch. Type-check clean (`tsc -p tsconfig.json` from `synthetic-nature/`); browser verification on the running :5200 pending (attach PDF → extract tables → merge CSV → downloads; scanned PDF → marker, no crash).

## [2026-09-15] — Agents access strip + revisitable drafts · header wordmark clip fix

Same uncommitted pre-push batch as the entry below (his "don't push before my approval"); live-verified on the :5200 dev server.

### Agents UI (`AgentsSection.tsx`) — access + draft resilience

- **Backend-access connect strip**: agent routes are vault-gated (401 without a master key or an `x-vault-token`), which reads as "broken" to a fresh clone. A slim strip now sits above the view switches — a coral dot when connected, security copy ("Agent routes are locked for security. Connect to prove this browser holds a provider key — no master key or terminal needed."), and a **Connect** button that clears any stale token, re-mints via `mintVaultToken()`, and refreshes on success. A blocked mint (no provider key in the vault) explains itself instead of failing silently.
- **Revisitable drafts**: the create-agent flow now survives a close/reload. Every field (step, description, urls, internet, name, system prompt, tools, schedule, picked skills/knowledge, the drafted manual + gather result) serializes to `localStorage['enzo.agents.draft']`; a coral **Resume draft** chip appears in the header when one exists, the modal hydrates from it on open, and a saved agent (or **Start over**) clears it.
- **1-second autosave**: a dirty-checked interval writes the draft snapshot at most once/sec (skips when there's nothing worth keeping yet, and when the serialized snapshot is byte-identical to the last write), with an "Autosaves to this device every second" / "Draft saved · <ago>" status line. Best-effort on quota.

### Ops — music search release blocker (diagnosis, no code change)

- **`/api/music-search` 404 on the running instance** was root-caused to a **stale backend**: the deployed image predates the music feature, so the route doesn't exist yet (the route + resolver are correct in source). No code change — the fix is operational: rebuild + restart the backend, and rebuild/republish `ghcr.io/theguysudo/enzo:latest` before the public push. Flagged separately: the resolver still leans on a single Piped instance (`api.piped.private.coffee`) — a multi-instance list or a keyless InnerTube fallback would make "clone it and music just works" robust.

## [2026-09-11 → 09-14] — Music player · marketplace card & info redesign · NYC subway theme · monochrome sweep

The whole batch sits **uncommitted in the working tree** (his call: "don't push before my approval"), live-verified on the dev servers (:5200 frontend, :5001 backend in docker-variant mode). One exclusion: the equalizer band-slider alignment fix (the "bars not aligned" report) is a separate in-flight item and is NOT part of this entry.

### The music player (Marketplace, 2026-09-13 → 09-14)

A floating music dock on the Marketplace surface — collapsed pill in the corner, expanding to a full player card (`src/components/MusicPlayer.tsx`, new). Search a song, click a result, it plays via the YouTube IFrame API. Keyless by design: no YouTube Data API, no quota.

- **Track search** — new backend route `GET /api/music-search?q=…` proxies the open-source YouTube frontends server-side (the browser can't call them — no CORS headers): Piped first, then Invidious. Instance lists rot fast, so `MUSIC_PIPED` / `MUSIC_INVIDIOUS` in `index.ts` are the documented tuning knob. As of 2026-09-13, a ~25-instance sweep found exactly ONE working Piped instance (`api.piped.private.coffee`) and every Invidious instance antibot-walled (Anubis) — so the array ships with just that one, and `{}` from the route means instance rot (not a bug), while 404 means the backend process is stale.
- **"For You" recommendations** — `src/lib/musicTaste.ts` is a device-local listening ledger (plays / completes / skips / replays / likes, keyed by title+channel, `localStorage` only — no PII, no network). A "For You" button sends one compact behavior digest to the new `POST /api/music-recommend`, where the user's own provider keys turn it into song seeds via an LLM call, then those seeds are searched and queued. No provider key → the button explains itself instead of failing silently.
- **Player card** — Disc hero (rotating vinyl while playing), track/artist with enter/exit animations, seek bar, prev/next walking the result queue, shuffle, loop, like, volume, a mini-EQ bars animation, zoom-out mode, keyboard controls scoped to the card (Space / arrows / S / L / E), and an always-mounted 1px IFrame host per YouTube ToS (playback survives collapse). Aesthetic: monochrome white, coral `#f0968a` strictly as the single accent.
- **Equalizer (shipped 2026-09-13, before the slider rework)** — 5-band Web Audio EQ (`src/lib/musicEq.ts`): BASS 80 Hz / LOW-MID 350 / MID 1400 / PRES 4000 / AIR 10 kHz, ±12 dB each, preamp ±12 dB, presets Flat / Bass Boost / Vocal Clarity / Warm / Crisp (any slider touch switches to Custom), persisted in `localStorage` (`enzo.music.eq`). "Enhance" starts OFF — normal playback is untouched until the user opts in. When ON, playback switches from the IFrame to a local engine: `AudioContext → MediaElementSource → preamp gain → 5 BiquadFilters → destination`. Because googlevideo URLs send no CORS headers (a tainted cross-origin source outputs silence through Web Audio), the backend adds `GET /api/music-stream/:videoId/audio` — a same-origin byte relay honoring Range requests (verified live: correct 206 + Content-Range). Stream resolution (`GET /api/music-stream/:videoId`) chains Piped `/streams` → innertube player API with the public ANDROID client key (itag 140, audio-only m4a ~128 kbps). **Honest limitation, verified live**: the PO-token era grants keyless URLs only the first ~1MB of the file; ranges past that 403. Most tracks are 3–8MB, so per-track the enhancer attempts the relay and **silently falls back to IFrame playback** with a small note in the EQ panel — playback never breaks. Read `clen` for total bytes, never probe the URL (a probe consumes the single range grant). The wall is swappable: a self-hosted cobalt resolver in `resolveAudioStream` lights the EQ up for every track with zero frontend changes. (The 2026-09-14 follow-up that replaced the rotated native sliders with Base UI vertical sliders and fixed their alignment is excluded from this entry per the user's instruction.)
- **New dependency**: `@base-ui/react` ^1.8.0 (the EQ slider component + `ui/card.tsx`) — the only package.json addition of the batch.

### Marketplace model-card + info redesign (2026-09-12 → 09-13)

- **New `ui/model-card.tsx`** replaces the old `InteractiveModelCard` grid. Each card carries the platform's own cover banner (`public/model_covers/` — nvidia / huggingface / openrouter / puter / groq / llm7, normalized 768×320; Pollinations has no published banner so it falls back to a brand plate + favicon). At rest covers are dimmed + grayscaled; hover restores the brand hue and scales 1.04 — colour as a response to the cursor instead of fifteen competing gradients. No cover reachable (air-gapped self-host, ad blocker) → a seeded brand-plate gradient carries the tile, so the grid never breaks. Provider logos come from Google's keyless favicon service (`sz=256`) keyed by domain, with `onError` hiding the img so the plate alone remains. Health dot (online white / offline coral / checking pulse) + latency live in a pill with a black backing (light banners would swallow a white dot). The action button (`btn-chitchat`) is a monochrome sweep-hover pill.
- **New `ui/card.tsx`** — a plain Card/CardHeader/CardContent/CardFooter primitive (the shadcn house pattern) backing the above.
- **New `lib/modelInfo.ts`** — keyless, in-browser background facts for a model from three public endpoints: OpenRouter's `/api/v1/models` (authoritative `hugging_face_id`, knowledge cutoff, Artificial Analysis intelligence/coding scores, richer prose), HuggingFace's repo API (exact downloads/likes/licence/pipeline/published), and DuckDuckGo Instant Answer (a Wikipedia-backed family paragraph — queried by family, not model name, because the index only covers encyclopedia topics). The repo is never guessed: an earlier name-search version returned confident lookalikes ("ALLAM-2-7B" → EdBerg/ALlama-2-7B, "alphafold2" → MurrellLab/AlphaFold2.jl), so the repo now comes only from an exact `hf/` id or OpenRouter's mapping; models with no public record say so.
- **Model info dialog** (`App.tsx`, portalled) — hovering a card and opening its info fetches `fetchModelInfo` and renders a full modal: prose summary (routing-boilerplate blurbs from the catalog are detected and dropped in favor of OpenRouter's), and a stats grid (downloads, likes, AA intelligence/coding, licence, task, knowledge cutoff, modality, tokenizer, published, context, max output, moderated, listed). Esc closes; the backdrop uses `will-change-transform` because the z-30 advisor pill's composited backdrop-filter layer punches through without it.
- **Grid entrance**: staggered GSAP entrance via `staggerIn` on `[data-model-card]`.
- **Terminal model chip** also re-skinned monochrome with a health-aware dot (online white / degraded coral / offline dark-coral), "Answering with <model>" copy naming the model, and slightly larger telemetry label.

### NYC subway marketplace theme (2026-09-11)

- New workspace theme **"NYC Subway Ride"** — his own AI image-to-video generation (GWR video MVP, 1080p) from the exact Unsplash reference photo (Juan Giraudo, "train door" J7zytkieZBs), cut to a 3.28MB 8.71s crossfade-seamless loop (CRF 23; seam-verified; the xfade head-trim recipe holds — any last-frame≠first-frame mismatch pops the seam). Wired end-to-end: `types.ts` (`nyc_subway` scene + theme entry), `MarketplaceCyberpunkSky.tsx` (per his "effects and lighting via code"), `HeaderThemeSelector` (Train icon), theme-remapping for persisted ids. Runtime shader additions: `uTremble` handheld camera shake (subway only), `uGrade` Unsplash-style monochrome grade (contrast pivot 0.36×1.16, glass-lift, bottom vignette, animated recording grain), playbackRate 1.0 for real footage (synthetic loops unchanged). Earlier CC-BY Zeodekken footage removed; `CREDITS.md` rewritten. **Docker lite variant keeps exactly the NYC subway loop** (`find … ! -name 'Subway_…_gwr_loop.mp4' -delete` — name-based, not the earlier `-size +5M` cutoff, which silently kept every small file: a 1MB homepage `_reversed` orphan whose forward partner was deleted would have shipped as a second, unreachable video; dry-run verified 24 mp4s → 1 kept / 23 deleted), so a basic `docker pull` of the lite image ships the NYC marketplace theme and no other video, while the ~292MB of anime loops stay full-only.
- **Legibility scrim**: marketplace video themes now sit under a flat `bg-black/65` (up from /40) — a gradient was rejected because the catalog scrolls, so any vertical falloff leaves some rows on a bright band.
- The 15MB raw source mp4 sits untracked at the repo root — **must be cleaned before commit**.

### Unsplash / Auto Wallpaper fully removed (2026-09-13)

His explicit ask, swept end to end: `AutoWallpaper.tsx` deleted, its Marketplace-sidebar settings block removed, backend proxy route + `src/features/unsplash.ts` deleted, `UNSPLASH_ACCESS_KEY` / `UNSPLASH_ENABLED` gone from `.env.example`, and the rows dropped from `docs/AGENTS.md` + `docs/README.md`. One orphan remains: `synthetic-nature/src/lib/unsplash.ts` is still tracked but now imports nothing — needs deletion before commit. Intentional "unsplash" grep survivors: the coding-preview free-CDN prompt line, CREDITS train-photo attribution, and this changelog's history.

### Monochrome micro-interaction sweep (2026-09-13 → 09-14)

The house palette tightened to monochrome + single coral accent across the ambient components:

- **`Switch.tsx`** — the 385-line styled-components build (eight nested divs painting a grid layer, a ripple, a progress arc, a status readout driven by classes nothing ever set, on a sluggish 2s transition, leaking a mint-green glow) replaced by a ~30-line monochrome track-and-thumb button.
- **`WeatherWidget.tsx`** — moved from the Marketplace filter-sidebar capsule to a compact chip pinned at the right of the "Unified Catalog." header row (hidden below md, same hover-expansion: temperature + city at rest, condition + humidity slide in on hover; IP geolocation → Open-Meteo with the 30-min sessionStorage cache unchanged).
- **`SystemPulse.tsx`** — ECG trace colors swapped green/red → white sweep on a faint white base, flatline in dark coral `#c96b62`, label to match; drop-shadow glows removed.
- **`LowPowerToggle.tsx`** — Lite/Full dot recolored amber/emerald → coral/white with a slightly larger mono label.
- **`HeaderThemeSelector`** — dropdown now portals to `<body>` with fixed coordinates recomputed on every open (the new scroll-collapsing nav is `overflow-hidden`, so an in-flow dropdown would be clipped), its own subtree counts as "inside" for click-outside, and `Train` icon + `nyc_subway` entry added to THEME_META.
- **Nav scroll-collapse** (App.tsx, 2026-09-13) — the pasted 21st.dev animated-nav recipe adapted into the existing floating header: scroll-down past 150px collapses the nav to a ~160px pill ("ENZO ☰"), scroll-up 80px springs it back open (asymmetric thresholds by design), click-to-reopen with the 80px rule preserved from wherever it was pressed; the ENZO brand and its DrawLineText animation untouched. A **native `scroll` listener** is used rather than framer's `useScroll` — ScrollTimeline never ticks in the in-app browser, so `change` events never fire; a wrapper div owns centering because framer writes the whole transform string; desktop-only below md. All four behaviors live-verified (collapse / 80px-climb re-expand / sub-threshold hold / click-pill).
- `package.json` dev script is now `tsx watch index.ts` (was a bare `tsx`), so the stale-backend 404s from live-testing sessions stop happening.
